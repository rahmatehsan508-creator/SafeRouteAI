    ...
    <div className="space-y-5">
      ...
      <div className="glass-panel-elevated rounded-2xl p-5 border border-cyan-500/20 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="glass-card px-3.5 py-2 rounded-xl border border-white/[0.08] text-center">
              <div className="text-xs text-slate-400 font-mono">Active Hazard Feeds</div>
            ...
            <div className="glass-card px-3.5 py-2 rounded-xl border border-white/[0.08] text-center">
              <div className="text-xs text-slate-400 font-mono">Community Verified</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
          {telemetryFactors.map((factor, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`glass-card p-3.5 rounded-2xl border border-white/[0.08] ${factor.border} transition-all duration-200 space-y-2 flex flex-col justify-between`}
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-white/[0.05] border border-white/10 text-slate-300">
                      {factor.weight}
                    </span>
                  </div>
                  <div className="text-xs font-bold text-slate-100">{factor.title}</div>
      ...
      <div className="space-y-2.5">
        <div className="flex items-center justify-between px-1">
          ...
          <span className="text-[11px] text-slate-400 font-mono">Click to evaluate safety in 1 click</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {popularCorridors.map((corridor, idx) => (
            <button
              ...
              className="glass-card hover:bg-[#0c152d]/90 p-4 rounded-2xl border border-white/[0.08] hover:border-cyan-400/40 text-left transition-all duration-200 group flex flex-col justify-between space-y-3"
            >
              <div>
                ...
                <p className="text-[11px] text-slate-400 font-light leading-snug mb-2">
                  {corridor.desc}
                </p>
              </div>
