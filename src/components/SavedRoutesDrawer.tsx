      ...
      <div className="w-full max-w-md glass-panel-elevated h-full flex flex-col shadow-[0_0_50px_rgba(0,0,0,0.8)] border-l border-white/15">
        ...
        <div className="p-4 sm:p-5 border-b border-white/[0.08] flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold text-slate-100 drop-shadow-sm">Saved Safe Routes</h2>
            <span className="bg-white/[0.08] border border-white/10 text-emerald-300 text-[10px] font-mono px-2 py-0.5 rounded-full font-bold">
              {savedRoutes.length}
            </span>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-3.5">
          {savedRoutes.length === 0 ? (
            <div className="py-20 text-center text-slate-400 space-y-3">
              ...
              <div className="text-sm font-bold text-slate-200">No Saved Routes Yet</div>
            ...
            savedRoutes.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="glass-card rounded-2xl p-4 space-y-3 border border-white/10 hover:border-emerald-400/40 transition-all duration-200 shadow-lg"
                >
                  {/* Origin & Destination */}
                  <div className="flex items-start justify-between gap-2">
                    <div className="space-y-1.5 text-xs">
                      <div className="font-semibold text-slate-200 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.8)] shrink-0"></span>
                        <span className="truncate max-w-[240px] drop-shadow-sm">{item.source}</span>
                      </div>
                      <div className="font-semibold text-slate-200 flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-rose-400 shadow-[0_0_8px_rgba(244,63,94,0.8)] shrink-0"></span>
                    ...
                    <button
                      ...
                      className="p-1.5 text-slate-400 hover:text-rose-300 hover:bg-rose-500/20 rounded-xl transition-colors border border-transparent hover:border-rose-400/30"
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-400 pt-2 border-t border-white/[0.06]">
                    <div className="flex items-center gap-2 font-light">
                      <span className="capitalize text-slate-300">{item.travelMode.replace('_', ' ')}</span>
                      <span>&bull;</span>
                    ...
                    <div className="text-emerald-300 font-bold flex items-center gap-1 bg-emerald-500/15 border border-emerald-400/30 px-2 py-0.5 rounded-lg text-xs">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    ...
                    <button
                      ...
                      className="bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-400/40 text-xs px-3.5 py-1.5 rounded-xl flex items-center gap-1.5 font-bold transition-all shadow-[0_0_10px_rgba(16,185,129,0.2)] hover:scale-105 active:scale-95"
                    >
